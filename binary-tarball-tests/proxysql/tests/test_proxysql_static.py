...
proxysql_executables, proxysql_files, proxysql_symlinks = get_artifact_sets()

def test_executables_exist(host):
    for executable in proxysql_executables:
...
def test_mysql_version(host):
    if proxysql_version_major in ['5.7', '5.6']:
        ...
        if (pro):
            expected = (
                'mysql  Ver ' + proxysql_version_pro_percona+'-pro for Linux on x86_64 (Percona XtraDB Cluster Pro binary (GPL) ' +
                proxysql_version_percona + ', Revision ' + proxysql_revision + ', WSREP version ' + wsrep_version + ')'
            )
            assert expected in host.check_output(base_dir+'/bin/mysql --version')
        else:
            expected = (
                'mysql  Ver ' + proxysql_version + ' for Linux on x86_64 (Percona XtraDB Cluster binary (GPL) ' +
                proxysql_version_percona + ', Revision ' + proxysql_revision + ', WSREP version ' + wsrep_version + ')'
            )
            assert expected in host.check_output(base_dir+'/bin/mysql --version')
...
def test_mysqld_version(host):
    if proxysql_version_major in ['5.7','5.6']:
        ...
        if (pro):
            expected = (
                'mysqld  Ver ' + proxysql_version_pro_percona+ '-pro for Linux on x86_64 (Percona XtraDB Cluster Pro binary (GPL) ' +
                proxysql_version_percona + ', Revision ' + proxysql_revision + ', WSREP version ' + wsrep_version + ')'
            )
            assert expected in host.check_output(base_dir+'/bin/mysqld --version')
        else:
            expected = (
                'mysqld  Ver ' + proxysql_version + ' for Linux on x86_64 (Percona XtraDB Cluster binary (GPL) ' +
                proxysql_version_percona + ', Revision ' + proxysql_revision + ', WSREP version ' + wsrep_version + ')'
            )
            assert expected in host.check_output(base_dir+'/bin/mysqld --version')
...
def test_symlinks(host):
    for symlink in proxysql_symlinks:
